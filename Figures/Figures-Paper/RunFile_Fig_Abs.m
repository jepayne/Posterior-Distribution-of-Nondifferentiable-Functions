...
m = 200; % Number of samples generated (either as bootstrap draws or posterior draws) [Set to 1000 in paper]
nZ = 200; % Number of draws to estimate parametric bootstrap [Set to 2000 in paper]
I = 200; % Number of draws from MCMC [Set to 1000 in paper]
IB = 200; %Number of bootstrap repetitions [Set to 2000 in paper]
sthetaML=-3:.5:3; % thetaML: Range of MLE values at which posterior is evaluated
...
quantgamma = 1; % If 1, then calculate the quantile plots for Gamma priors
covgamma = 1; % If 1, then calculate the coverage plots for Gamma priors
quantbeta = 1; % If 1, then calculate the quantile plots for Beta priors
...
% 2.2. Calculate quantiles for a set vector of data realisation:
% ------------------------------------------------------------
if quantgamma == 1
    % 2.2.1.) Set up the containers
    thetadata=zeros(m,1);
    ...
    % 2.2.2.) Bootstrap and Asymptotic Approximation (Two Sided)
    figure(ifig)
    step=sthetaML(1,end)-sthetaML(1,end-1);
...
% % 3.2. Calculate quantiles for a set vector of data realisation:
% % ------------------------------------------------------------
if quantbeta == 1
